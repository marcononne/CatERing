...
public class Test2a {
    ...
    public static void main(String[] args) throws UseCaseLogicException {
        if(CatERing.getInstance().getUserManager().fakeLogin("Alessandro")) {
            ...
            try {
                System.out.println("\n\n\t--About to create a task referencing procedure with id = " + suppli.getId() + "--");
                Task suppliTask = CatERing.getInstance().getTaskManager().addTask(suppli);
                System.out.println(suppliTask.toString());
            } catch (UseCaseLogicException e) {
                System.out.println(e.getMessage());
            }
            ...
            try {
                ...
                ArrayList<Task> tasks = CatERing.getInstance().getTaskManager().getCurrentSheet().getTasks();
                for(Task t: tasks) {
                    System.out.println(t.toString());
                }
                assert pizzaTask != null;
                System.out.println("\n\n\t--Now let's remove task with id: " + pizzaTask.getId() + "--");

                CatERing.getInstance().getTaskManager().removeTask(pizzaTask);

                System.out.println("\n\n\t--Tasks in the current summary sheet after removing task "+pizzaTask.getId()+":--");
                tasks = CatERing.getInstance().getTaskManager().getCurrentSheet().getTasks();
                for(Task t: tasks) {
                    System.out.println(t.toString());
                }
